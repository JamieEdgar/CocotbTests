...
def half_stream_accumulate():
    ...
    sources = [ files_path / "leading_zero_count.sv",
                ...
                proj_path / "half_stream_accumulate.sv"
                ]

    runner = get_runner(sim)
    runner.build(
        sources=sources,
        hdl_toplevel="half_stream_accumulate",
        build_args=["--timing"]
    )

    runner.test(hdl_toplevel="half_stream_accumulate", test_module="half_stream_accumulate_test,")
