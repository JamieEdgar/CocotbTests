...
def half_stream_exponent():
    ...
    sources = [ files_path / "delay.sv",
                ...
                proj_path / "half_stream_exponent.sv"
                ]

    runner = get_runner(sim)
    runner.build(
        sources=sources,
        hdl_toplevel="half_stream_exponent",
        build_args=["--timing"],
        waves=True
    )

    runner.test(hdl_toplevel="half_stream_exponent", test_module="half_stream_exponent_test,",
                waves=True)


if __name__ == "__main__":
    half_stream_exponent()
