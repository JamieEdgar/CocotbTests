...
async def test_vectors(dut, mults, b):
    ...
    for i in range(c):
        for j in range(mults):
            dut.vector_b[j].value = RealToHalf(b[i * mults+j]) # vector should go in in order
...
def calc_w1_matrix(data_halves):
    W1_real = []
    for i in range(len(data_halves)):
        new_row = []
        for j in range(len(data_halves[0])):
            new_row.append(DoubleFromHalf(data_halves[i][j]))
        W1_real.append(new_row)
    #print(W1_real)
    return W1_real
...
async def mnist_test(dut):
    ...
    W1_real = calc_w1_matrix(W1)
    await load_matrix_halves(dut, 50, 784, 2, W1)
    image = LoadImage(image_filename, 0)
    image = BytesToReal(image)
    print("image", image)
    print("max image", np.max(image))
    await test_vectors(dut, 2, image)
    dut.in_valid.value = 0;
    for i in range(100):
        await toggle_clk(dut)
    calc_result = np.dot(W1_real, image)
    print("calc_result", calc_result)
    print("min W1", np.min(W1_real))
    print("max W1", np.max(W1_real))
